...
class MCTSNode:
    ...
    def select_child(self, c_param):
        ...
        for child in self.children:
            ...
            exploration = c_param * math.sqrt(2 * math.log(self.visits) / child.visits) if child.visits > 0 else float("inf")
    ...
    def expand(self):
        if self.is_leaf():
            # Expand by generating all possible neighboring join orders
            neighbors = neighborhood(self.state)
            for neighbor in neighbors:
                child_node = MCTSNode(neighbor)
                self.children.append(child_node)
                child_node.parent = self
            self.expanded = True

    def simulate(self, parsed_query):
        import random
        # Simulate by selecting a random neighbor and estimating the reward
        neighbors = neighborhood(self.state)

        random_neighbor = random.choice(neighbors)
        reward = get_join_order_cost(parsed_query,random_neighbor)
        return 1/reward
    ...
    def backpropagate(self, reward):
        # Backpropagate by updating visits and rewards along the path to the root
        current_node = self
...
def mcts_search(query, num_iterations, c_param):
    ...
    root_node = MCTSNode(tables)

    for i in range(num_iterations):
        # Selection
        current_node = root_node
        while not current_node.is_leaf():
            current_node = current_node.select_child(c_param)
        ...
        # Simulation
        reward = current_node.simulate(parsed_query)

        # Backpropagation
        current_node.backpropagate(reward)

    best_child = root_node.select_child(0)  # Select child with highest reward
